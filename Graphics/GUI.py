...
def animate(i):
    ...
    try:
        ...
        data = mesh.Mesh.from_file('Rocket.stl')

        data.rotate([1,0,0],np.radians(-float(ori[1])))
        data.rotate([0,1,0],np.radians(float(ori[0])))
        data.rotate([0,0,1],np.radians(float(ori[2])))
        ...
        warn("Waiting for connection. Disconect!")
